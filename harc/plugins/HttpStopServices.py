...
class HttpStopServices(Plugable):
    def __init__(self):
    ...
    def execute(arguments, settings, properties):
        ...
        for project in projects:
            ...
            if status_code != 200:
                message = "status_code: " + str(response.status_code) + " reason: " + response.reason + ", content: " + response.content + ", content: " + response.content
                raise PluginException(message)
            ...
            for service in services:
                ...
                message = {"command": "service:stop", "session": session, "-s": service['name'], "-e": environment, "-n": project['name']}
                headers = {'Content-Type': 'application/json'}
                response = post(server + '/job', json.dumps(message), headers=headers)
                status_code = response.status_code
                if status_code != 200:
                    message = "service: " + service['name'] + ", status_code: " + str(response.status_code) + ", reason: " + response.reason + ", content: " + response.content + ", content: " + response.content
                    raise PluginException(message)
